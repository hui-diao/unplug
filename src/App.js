      ...
      <div style={S.setupWrap}>
        ...
        <div style={{ ...S.fieldGroup, ...(err && totalMin < 1 ? S.fieldErr : {}) }}>
          ...
          <div style={S.spinnerRow}>
            ...
            <Spinner value={mins} min={0} max={55} step={5} label="分" onChange={setMins} />
          </div>
          <div style={S.fieldLine} />
        </div>
...
export default function App() {
  ...
  const handleStart = (s) => {
    setSession({ ...s, startTime: Date.now() });
  ...
  return (
    <>
      ...
      {screen === "focus-timer" && session && (
        <FocusTimer
          session={session}
          onDone={() => { setGaveUp(false); setScreen("done"); }}
          onGiveUp={() => { setGaveUp(true); setScreen("done"); }}
        />
      )}
